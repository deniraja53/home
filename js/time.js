...
const config = {
  greetings: [
    { hour: 6, text: "Selamat Dini Hari" },
    { hour: 9, text: "Selamat Pagi" },
    { hour: 12, text: "Selamat Siang" },
    { hour: 14, text: "Selamat Siang" },
    { hour: 17, text: "Selamat Sore" },
    { hour: 19, text: "Selamat Malam" },
    { hour: 22, text: "Selamat Malam" },
    { hour: 24, text: "Selamat Tengah Malam" },
  ],
  anniversaries: {
    5.12: "汶川大地震纪念日",
...
const timeProcessor = {
  getGreeting() {
    ...
    const greeting =
      config.greetings.find((g) => hour < g.hour) ||
      config.greetings[config.greetings.length - 1];
  ...
  updateClock() {
    ...
    if (domCache.time) {
      domCache.time.innerHTML = `${y}&nbsp;-&nbsp;${mm}&nbsp;-&nbsp;${d}&nbsp;&nbsp;<span class='weekday'>${weekday[day]}</span><br><span class='time-text'>${h}:${m}:${s}</span>`;
    }
  },
  ...
  calculateProgress() {
    ...
    const yearProgress = {
      ...
      percent: utils.calculatePercentage(
        yearPassDays,
        yearPassDays + yearRemainingDays
      ),
    };
  ...
  updateProgressBars(progress) {
    const { todayProgress, weekProgress, monthProgress, yearProgress } =
      progress;
...
const anniversaryProcessor = {
  ...
  showAnniversaryMessage(key) {
    if (domCache.change)
      domCache.change.innerHTML = "Silence&nbsp;in&nbsp;silence";
    if (domCache.change1)
      domCache.change1.innerHTML = "今天是" + config.anniversaries[key];
